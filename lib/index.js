...
const newTransport = () => {
	if (environment.isProduction() && environment.logentries.token) {
		return new winston.transports.Insight({
...
const defaultLogLevel = environment.logger.loglevel || 'debug'

module.exports.getLogger = _.memoize((filename, logLevel = defaultLogLevel) => {
	return new Logger(logLevel, filename, transport)
}, (filename, level) => {
	return level
		? `${level}:${filename}`
		: `${defaultLogLevel}:${filename}`
})
